...
class VFARoIHead(MetaRCNNRoIHead):
    ...
    def _bbox_forward_train_cropped(self, query_feats: List[Tensor],
                            support_feats: List[Tensor],
                            sampling_results: object,
                            query_img_metas: List[Dict],
                            query_gt_bboxes: List[Tensor],
        ...
        support_roi_feats = self.extract_support_roi_feat(support_feats, support_rois)[0]
        # support_feats[0].shape: torch.Size([15, 2048])
        # support_roi_feats.shape: torch.Size([15, 1024, 14, 14])

        query_rois = bbox2roi([res.bboxes for res in sampling_results])
        query_roi_feats = self.extract_query_roi_feat(query_feats, query_rois)
        #### Replace support_feat with "support_roi_feats"
        # support_feat = self.extract_support_feats(support_feats)[0]
        support_feat_rec, support_feat_inv, _, mu, log_var = self.vae(support_roi_feats)
        ...
        for img_id in range(batch_size):
            ...
            random_index = np.random.choice(
                range(len(support_gt_labels)))
            random_query_label = support_gt_labels[random_index]
            for i in range(support_roi_feats.size(0)):
                if support_gt_labels[i] == random_query_label:
                    bbox_results = self._bbox_forward(
                        query_roi_feats[start:end],
                        support_feat_inv[i].sigmoid().unsqueeze(0))
        ...
        loss_vae = self.vae.loss_function(
            support_roi_feats, support_feat_rec, mu, log_var)
        loss_bbox.update(loss_vae)

        bbox_results.update(loss_bbox=loss_bbox)
        return bbox_results
    
    def _bbox_forward_train_cropped_ver2(self, query_feats: List[Tensor],
                            support_feats: List[Tensor],
                            sampling_results: object,
                            query_img_metas: List[Dict],
                            query_gt_bboxes: List[Tensor],
        ...
        """
        support_rois = bbox2roi(support_gt_bboxes)        
        # 윈래는 전역정보를 고려해야해서 pooling했지만 지금은 부적합할수도?
        # support_roi_feats = self.extract_support_roi_feat(support_feats, support_rois)[0]
        support_roi_feats = self.extract_query_roi_feat(support_feats, support_rois)
        # support_feats[0].shape: torch.Size([15, 2048])
        # support_roi_feats.shape: torch.Size([15, 1024, 14, 14])
    ...
    def simple_test_bboxes(
        ...
        for i in range(len(proposals)):
            end = start + proposals[0].size(0)
            this_query_roi = query_roi_feats[start:end]
            ...
            det_feats.append(this_query_roi)
        return det_bboxes, det_labels


    def simple_test_bboxes_novel(
            self,
            query_feats: List[Tensor],
            support_feats_dict: Dict,
            query_img_metas: List[Dict],
            ...
            rcnn_test_cfg: ConfigDict,
            rescale: bool = False,
            novel_cls=20) -> Tuple[List[Tensor], List[Tensor]]:
        img_shapes = tuple(meta['img_shape'] for meta in query_img_metas)
        scale_factors = tuple(meta['scale_factor'] for meta in query_img_metas)
        ...
        start = 0
        for i in range(len(proposals)):
            end = start + proposals[i].size(0)
            this_query_roi = query_roi_feats[start:end]
            prd_cls = torch.argmax(cls_score[i], dim=1)
            novel_mask = prd_cls == novel_cls
            det_bbox, det_label = self.bbox_head.get_bboxes(
                rois[i],
                cls_score[i],
                bbox_pred[i],
            ...
            if sum(novel_mask)>0:
                this_feats = torch.mean(this_query_roi[novel_mask], dim=0)
            else:
                this_feats = torch.zeros_like(this_query_roi[0])
            det_feats.append(this_feats)
